...
try:
    ...
    # SETUP
    # =========================
    print("=== SIMULADOR HUSKY: TRAJETÓRIA REAL-TIME ===")
    ...
    # Flag para mandar sinal de colisão para o Node-RED
    ...
    while True:
        ...
        metric_dist += d_step
        if not collision_event_flag: # Só soma dist sem impacto se não bateu agora
            metric_dist_no_crash += d_step
        last_pos = pos[:]

        # === CORREÇÃO: Só checa colisão se a missão NÃO acabou ===
        collision_event_flag = False # Reseta a flag a cada frame
            ...
            if len(p.getContactPoints(robot)) > 4 and col_cd == 0:
                    for c in p.getContactPoints(robot):
                        if c[2] != plane: 
                            ...
                            col_cd = 50 
                            metric_dist_no_crash = 0.0
                            collision_event_flag = True # AVISA O NODE-RED!
                            break
        ...
        if now - last_log > 0.15: 
            ...
            # Erro lateral estimado (linha reta start->goal)
            ...
            # JSON COMPLETO COM FLAG DE COLISÃO
            safe_publish({
                "status": status, 
                "colisoes": metric_col, 
                "dist_sem_impacto": round(metric_dist_no_crash, 2),
                "tempo_reacao_s": round(reaction, 3), 
                "erro_lat_medio": round(lat_err, 2),
                "sensores": [round(x,1) for x in readings], 
                "motor": [round(cmd_l,1), round(cmd_r,1)],
                "pos": {"x": round(pos[0], 2), "y": round(pos[1], 2)},
                "evento_colisao": collision_event_flag # NOVA FLAG IMPORTANTE
            })
            last_log = now
            if not mission_complete:
